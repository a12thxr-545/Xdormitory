...
export default function AuthModal({ isOpen, onClose, onSuccess }: AuthModalProps) {
  ...
  const [role] = useState<'user' | 'staff'>('user');
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-3.5">
          ...
          <div>
            ...
            {mode === 'register' && (
              <p className="mt-1 text-[10px] text-slate-500">
                * ต้องมีทั้งตัวอักษรพิมพ์เล็ก (a-z) และพิมพ์ใหญ่ (A-Z)
              </p>
            )}
          </div>
          ...
          {mode === 'register' && (
            <>
              ...
              <div className="p-3 rounded-2xl bg-slate-50 border border-slate-200 text-[11px] text-slate-700 flex items-start gap-2">
                <AlertCircle className="w-4 h-4 text-slate-600 shrink-0 mt-0.5" />
                <span>
                  การสมัครสมาชิกเป็น<strong>บัญชีผู้เช่า (User)</strong> เท่านั้น การแต่งตั้งสิทธิ์<strong>พนักงาน (Staff)</strong> จะต้องดำเนินการผ่านหน้าหลังบ้าน (Database Manager) เพื่อความปลอดภัย
                </span>
              </div>
            </>
          )}
